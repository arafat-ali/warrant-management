    ...
    <div id="home">
      <div class="flex flex-wrap -mx-3">
        <div class="w-full px-3">
          ...
          <!-- Dashboard Start -->
          <div v-show="isFormSubmit" class="w-full px-4 mb-8">
            <div class="flex justify-between -mx-3 mb-8">
              <div @click="showAllWarrantsTableFunc" class="cursor-pointer w-1/2 xl:w-1/4">
                <div class="w-full bg-white border text-blue-400 rounded-lg flex items-center p-6 mb-6 xl:mb-0">
                  ...
                  <div class="text-gray-700">
                    <p class="font-semibold text-3xl">{{ totalWarrants.length }}</p>
                    <p class="text-sm">মোট ওয়ারেন্ট</p>
                  </div>
                </div>
              </div>

              <div @click="showPendingWarrantsTableFunc" class="cursor-pointer w-1/2 xl:w-1/4">
                <div class="w-full bg-white border text-yellow-400 rounded-lg flex items-center p-6 mb-6 xl:mb-0">
                  ...
                  <div class="text-gray-700">
                    <p class="font-semibold text-3xl"> {{ totalPendingWarrant.length }} </p>
                    <p class="text-sm">মোট মুলতবি ওয়ারেন্ট</p>
                  </div>
                </div>
              </div>

              <div @click="showCompletedWarrantsTableFunc" class="cursor-pointer w-1/2 xl:w-1/4 ">
                <div class="w-full bg-white border text-green-400 rounded-lg flex items-center p-6" >
                  ...
                  <div class="text-gray-700">
                    <p class="font-semibold text-3xl"> {{ totalCompletedWarrant.length }} </p>
          ...
          <!-- Table Start -->
          <div v-show="showAllWarrantsTable" class="w-full bg-white border rounded-lg p-4">
                ...
                <div class="h-96  overflow-x-auto">
                  <div class="align-middle inline-block min-w-full overflow-hidden sm:rounded-lg border-b">
                    ...
                    <table class="min-w-full">
                      ...
                      <tbody class="text-xs">
                        <tr class="border-l border-r" v-for="warrant in totalWarrants" :key="warrant.id">
                          <td class="p-2  border-b border-gray-200">{{warrant.process_number}}</td>
                          <td class="p-2  border-b border-gray-200">{{warrant.case_section_and_date}}</td>
                          <td class="p-2  border-b border-gray-200">{{warrant.criminal_name}}</td>
                          <td class="p-2  border-b border-gray-200">{{warrant.criminal_father_name}}</td>
                          <td class="p-2  border-b border-gray-200">{{warrant.criminal_address}}</td>
                          <td class="p-2  border-b border-gray-200">{{warrant.warrant_type}}</td>
                          ...
                          <td class="p-2  border-b border-gray-200">{{warrant.arrest_warrant_received_to_thana | moment("from", "now", true) }}</td>
                        </tr>
                      </tbody>
                    </table>
                  </div>
          ...
          <!-- Table End -->
          <!-- Table Start -->
          <div v-show="showTotalPendingWarrant" class="w-full bg-white border rounded-lg p-4">
            <div class="flex flex-col">
                <div class=" h-96  overflow-x-auto">
                  <div class="align-middle inline-block min-w-full overflow-hidden sm:rounded-lg border-b">
                    <h1>মোট মুলতবি ওয়ারেন্ট</h1>
                    <table class="min-w-full">
                      <thead class="">
                          <tr class="text-xs ">
                            <th class="px-6 py-3 border bg-gray-50 text-left text-xs leading-4 tracking-wider font-semibold">প্রসেস নং</th>
                      ...
                      <tbody class="text-xs">
                        <tr class="border-l border-r" v-for="warrant in totalPendingWarrant" :key="warrant.id">
                          <td class="p-2  border-b border-gray-200">{{warrant.process_number}}</td>
                          <td class="p-2  border-b border-gray-200">{{warrant.case_section_and_date}}</td>
                          <td class="p-2  border-b border-gray-200">{{warrant.criminal_name}}</td>
                          <td class="p-2  border-b border-gray-200">{{warrant.criminal_father_name}}</td>
          ...
          <!-- Table End -->
          <!-- Table Start -->
          <div v-show="showTotalCompletedWarrant" class="w-full bg-white border rounded-lg p-4">
            <div class="flex flex-col">
                <div class=" h-96  overflow-x-auto">
                  <div class="align-middle inline-block min-w-full overflow-hidden sm:rounded-lg border-b">
                    <h1>মোট তামিল ওয়ারেন্ট</h1>
                    <table class="min-w-full">
                      <thead class="">
                          <tr class="text-xs ">
                            <th class="px-6 py-3 border bg-gray-50 text-left text-xs leading-4 tracking-wider font-semibold">প্রসেস নং</th>
                      ...
                      <tbody class="text-xs">
                        <tr class="border-l border-r" v-for="warrant in totalCompletedWarrant" :key="warrant.id">
                          <td class="p-2  border-b border-gray-200">{{warrant.process_number}}</td>
                          <td class="p-2  border-b border-gray-200">{{warrant.case_section_and_date}}</td>
                          <td class="p-2  border-b border-gray-200">{{warrant.criminal_name}}</td>
                          <td class="p-2  border-b border-gray-200">{{warrant.criminal_father_name}}</td>
  ...
  export default {
    ...
    data () {
      return {
        ...
        isFormSubmit:false,
        showAllWarrantsTable:false,
        showTotalPendingWarrant:false,
        showTotalCompletedWarrant:false
    ...
    methods:{
      ...
      formSubmit(){
        ...
        axios
        .get("api/get-si-performence-data/" + this.siId+'/'+this.start_date+'/'+this.end_date)
        .then(response => {
          let data = response.data.data;
          this.totalWarrants = data.totalWarrant ? data.totalWarrant : 0;
      ...
      showAllWarrantsTableFunc(){
        this.showAllWarrantsTable = !this.showAllWarrantsTable;
        this.showTotalPendingWarrant = false;
        this.showTotalCompletedWarrant = false;
      },
      showPendingWarrantsTableFunc(){
        this.showTotalPendingWarrant = !this.showTotalPendingWarrant;
        this.showAllWarrantsTable = false;
        this.showTotalCompletedWarrant = false;
      },
      showCompletedWarrantsTableFunc(){
        this.showTotalCompletedWarrant = !this.showTotalCompletedWarrant;
        this.showAllWarrantsTable = false;
        this.showTotalPendingWarrant = false;
      }

      ...
      <div class="w-full px-3">
        ...
        <div class="w-full bg-white border rounded-lg px-8 py-6 mb-8 xl:mb-0 ">
          <div class="flex flex-col gap-1 lg:flex-row justify-between items-center lg:gap-0">
            <div class="text-sm text-gray-400 font-medium"> Showing: {{ searchedWarrant.length }} unreceieved warrants</div>
                ...
                <tbody class="text-sm">
                  <tr class="border-l border-r" v-for="(warrant,index) in searchedWarrant" :key="warrant.id">
                    ...
                    <td class="p-2  border-b border-gray-200 text-center">
                      <button class="bg-blue-500 p-3 text-white text-sm text-center rounded-md hover:bg-blue-700" @click="receiveWarrant(warrant.id)">রিসিভ</button>
                    </td>
                  </tr>
                </tbody>
              </table>
...
export default {
  ...
  data() {
    return {
      ...
      fields: [
        ...
        { id: "4", name: "কোর্ট হতে প্রেরণের তারিখ", type:'date', nameArr: "arrest_warrant_to_thana" },
  ...
  methods: {
    ...
    receiveWarrant(warrantId){
      axios
        .get("api/receive-recall-ci/"+warrantId)
        .then((response) => {
          console.log(response);
          this.getWarrant();
          //alert('');
    ...
    getWarrant(){
      axios
        .get("api/get-recalled-warrant-ci")
        .then((response) => {
          console.log(response);
